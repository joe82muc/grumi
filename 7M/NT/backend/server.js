...
const DATA_DIR = process.env.NT_DATA_DIR || path.join(__dirname, "data");
...
app.use((req, res, next) => {
  ...
  if (origin && (/^https?:\/\/localhost(:\d+)?$/.test(origin) || /^https?:\/\/127\.0\.0\.1(:\d+)?$/.test(origin) || origin === "https://joe82muc.github.io")) {
...
for (const file of ["index.html","content.js","app.js","style.css","probe.html","probe.js","lehrer.html","lehrer.js"]) {
  app.get("/"+file, (_req,res) => res.sendFile(path.join(staticRoot,file)));
}
app.use("/assets", express.static(path.join(staticRoot,"assets"),{fallthrough:false}));

if (require.main === module) app.listen(PORT, () => console.log(`NT7 server on http://localhost:${PORT}`));
module.exports = app;
