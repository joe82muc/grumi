(function () {
  ...
  function renderNav(active = new URLSearchParams(location.search).get("teil")) {
    const done = content.units.filter(u => progress[u.id]?.done).length;
    document.getElementById("progress-total").textContent = `${done} / ${content.units.length}`;
  ...
  function renderUnit(u) {
    ...
    const checked = (progress[u.id]?.checks || []).filter(Boolean).length;
  ...
  function check(u, i) {
    ...
    document.getElementById("unit-count").textContent = `${(progress[u.id]?.checks || []).filter(Boolean).length} / ${u.checks.length} Aufgaben bearbeitet`;
  }
