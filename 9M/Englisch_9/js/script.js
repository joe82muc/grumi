...
function hasWhenBaseVerbError(text) {
  var val = String(text || '').toLowerCase();
  return /\bwhen\b[^.?!]{0,60}\b(come|go|see|eat|take|get|make|do|have|run|write|read|drive|buy|meet|leave|find|say|tell)\b/.test(val);
}
async function aiCheckQuality(answer, task, minWords) {
  try {
    var res = await fetch(API_BASE + '/api/check-quality', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ answer: answer, task: task || '', minWords: minWords || 6 })
    });
    if (!res.ok) throw new Error('HTTP ' + res.status);
    return await res.json();
  } catch (_e) {
    return { correct: false, verdict: 'Noch nicht vollständig', reason: 'KI-Pruefung gerade nicht erreichbar.' };
  }
}
async function checkFree(id) {
  if (answered[id]) return;
  var inp=$(id+'i')||$(id+'-i');
  var raw = inp.value || '';
  ...
  var aiReason = '';
  if (!ok && raw.trim().length >= 12 && !(PAGE==='pg' && hasWhenBaseVerbError(raw))) {
    var ai = await aiCheckQuality(raw, (PAGE==='pg' ? 'Past Progressive free writing' : 'Present Perfect free writing'), 6);
    if (ai && ai.correct === true) ok = true;
    aiReason = ai && ai.reason ? String(ai.reason) : '';
  }

  inp.classList.remove('correct','wrong'); inp.classList.add(ok?'correct':'wrong');
  var fb=$(id+'f')||$(id+'-fb');
  fb.className = $(id+'-fb') ? 'fb-msg '+(ok?'good':'bad') : 'fb '+(ok?'good':'bad');
  fb.textContent = ok ? 'Richtig' : 'Noch nicht vollständig';
  if (!ok && PAGE==='pg' && hasWhenBaseVerbError(raw)) fb.textContent += ' - Nach when brauchst du Simple Past (z. B. came statt come).'; else if (!ok && aiReason) fb.textContent += ' - ' + aiReason;
  answered[id]=true;
