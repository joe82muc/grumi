(() => {
  ...
  // Wird nur im fehlerbezogenen Tipp gezeigt, wenn die KI einen Fehler meldet.
  const hintTopicVideos = {
    ...
    "kegel-volumen": { id: "IsPM8oH5Cmw", label: "Kegel: Volumen" },
    "kegel-oberflaeche": { id: "OGbBx5mPju8", label: "Kegel: Oberfläche" },
    "kegel-mantellinie": { id: "FECtVbC-mgk", label: "Satz des Pythagoras" },
    "zylinder-volumen": { id: "6sf_cvf4xxE", label: "Zylinder: Volumen" },
  ...
  // Eine einzelne Prüfzeile der KI hübsch aufbereiten. Jede Zeile darf mit einem
  // Status-Marker beginnen: [ok] grün (richtig), [fehler] rot (falsch),
  // [fehlt] orange (fehlt noch). Ohne Marker richtet sich die Farbe nach dem
  // Gesamtstatus (kind). So wird pro Zeile einzeln grün/rot/orange markiert.
  function formatFeedbackLine(rawLine, kind) {
    let line = String(rawLine).trim().replace(/^[-•*•]\s*/, "");
    line = line.replace(/[✓✔✅]+\s*$/u, "").trim();
    if (!line) return "";

    // Status-Marker am Zeilenanfang erkennen und entfernen.
    let marker = "";
    const markerMatch = line.match(/^\[(ok|fehler|fehlt|miss)\]\s*/i);
    if (markerMatch) {
    ...
    const isMissing = marker === "fehlt" || marker === "miss" || (!marker && /^fehlt\b/i.test(line));
    let cls = "fb-line";
    let icon = "•";
    if (marker === "ok") {
      cls = "fb-line ok";
      icon = "✓";
    } else if (marker === "fehler") {
      cls = "fb-line bad";
      icon = "✗";
    } else if (isMissing) {
      cls = "fb-line miss";
  ...
  function renderFeedbackFromServer(data, task, step) {
    const lines = String(data.analysis || "")
      ...
      .map((line) => line.trim())
      .filter(Boolean);
    // Nur bei Fehler einen passenden Tipp anbieten. Bevorzugt der von der KI
    // gelieferte fehlerbezogene Tipp (data.hint) samt passendem Video
    // (data.hintTopic). Fällt das weg, greift der schritt-basierte Tipp.
    let offerHint = null;
    if (!data.correct) {
